...
vif(model1)
#if the vif shows more than 10 it means that there is such a strong
#relationship between the variables like that these  may be collinear 
#if collinear this will be bias the results of the model from our results 
#we see that systolic and diastolic might be collinear 

#tolerance the inverse of vif; we run this as an extra check 
1/vif(model1)   
#with these results we look for the last column to be >.10, if it's less
#than this means it's collinear, the two variables are systolic and diastolic 
